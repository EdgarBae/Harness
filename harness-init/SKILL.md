...
각 Phase 내에서 에이전트는 아래 사이클을 따른다:

```
READ → SPECIFY → IMPLEMENT → GATE → EVOLVE
```

1. **READ**: 변경할 파일의 현재 상태 파악
2. **SPECIFY**: 무엇을 어떻게 바꿀지 명확히 한 후 착수
3. **IMPLEMENT**: 가장 단순한 형태로 먼저 구현 (Frugal-First)
4. **GATE**: 검증 명령어로 기계적 확인 (파싱, 임포트, 빌드, 테스트)
5. **EVOLVE**: 검증 통과 시 다음 항목으로 진화. 실패 시 수정 후 재검증

### 검증 수준 (Gate Levels)
- **Mechanical**: 코드 파싱, import 해소, 문법 에러 없음
- **Semantic**: 비즈니스 로직이 스펙과 일치
- **Integration**: 컴포넌트 간 연결이 end-to-end로 동작

### 핵심 규율
- **Frugal-First**: 가장 단순한 동작 구현 먼저. 불필요한 추상화 금지
- **진화, 반복 아님**: 이미 동작하는 코드는 다시 짜지 않는다. 실패 시 되돌리지 않고 수정
- **수렴 체크**: Phase 완료 시 구현 결과가 PRD 스펙에서 벗어나지 않았는지 확인

> Phase 실행 원칙은 [Ouroboros](https://github.com/Wonderfull-Wanderings/ouroboros) 방법론을 참고하여 harness에 맞게 재구성했다.
